...
{
    const char* in_pcm_file = NULL;
    const char* out_aac_file = NULL;
    FILE* infile = NULL;
    FILE* outfile = NULL;
   ...
   const char* codec_name = NULL;
    ...
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <input_file out_file [codec_name]>, argc:%d\n",
            argv[0], argc);
        //return 0;
        in_pcm_file = "..\\res\\48000_2_f32le_packetd.pcm";//aac 编码只支持AV_SAMPLE_FMT_FLTP
        out_aac_file = "..\\res\\48000_2_f32le_packetd_out.aac";
    }
    else {
    ...
    int framesize = frame->buf[0]->size + frame->buf[1]->size;// 从buf数组中获取到的size，每个通道数据总和
    if (ret < 0)
    {
    ...
    for (;;) {
        ...
        */
        ...
        // 设置pts
        pts += frame->nb_samples;// 不懂为什么要这样写？？？
