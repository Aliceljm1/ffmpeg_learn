...
// 播放：ffplay -pixel_format yuv420p -video_size 512x288 -framerate 25  out.yuv
// 错误的播放方式 会导致花屏，数据错乱 ffplay -pixel_format yuv420p -video_size 768x320 -framerate 25  out.yuv
int decode_video(int argc, char** argv)
{
    ...
    if (argc <= 2)
    {
        ...
        filename = "..\\res\\test.h264";
        outfilename = "..\\res\\out.yuv";
    }else{
        filename = argv[1];
        outfilename = argv[2];
    }
