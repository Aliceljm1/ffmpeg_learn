...
int decode_audio(int argc, char** argv)
{
    ...
    uint8_t* data = NULL;//准备解析的数据
    size_t   data_size = 0;//准备解析的数据大小
    AVPacket* pkt = NULL;
    ...
    while (data_size > 0)
    {
        ...
        //会根据解码器的类型，解析出一个完整的packet，核心就是确定data中下一个帧的边界，
        //此时的pkt仅仅data和size有值，其他的都是0 ，且第一次调用时pkt->data == data（ 不会生成新的内存），pkt.size就是第一帧的大小
        ...
        if (data_size < AUDIO_REFILL_THRESH)    // 如果数据少了则再次读取
        {
            ...
            data = inbuf;
            // 读取数据 长度: AUDIO_INBUF_SIZE - data_size ,确保总长度仍然是AUDIO_INBUF_SIZE
    ...
    pkt->size = 0;
    //解码器中可能还有一些数据，需要通过输入一个空数据包，它会告诉解码器没有更多的输入数据了，解码器应该开始输出所有剩余的帧
    decode(codec_ctx, pkt, decoded_frame, outfile);
