...
extern "C"
{
#include "../ffmpeg-4.2.1/include/libavutil/avassert.h"
#include "../ffmpeg-4.2.1/include/libavutil/frame.h"
#include "../ffmpeg-4.2.1/include/libavutil/channel_layout.h"
#include "../ffmpeg-4.2.1/include/libavutil/opt.h"
#include "../ffmpeg-4.2.1/include/libavutil/mathematics.h"
#include "../ffmpeg-4.2.1/include/libavutil/timestamp.h"
#include "../ffmpeg-4.2.1/include/libavformat/avformat.h"
#include "../ffmpeg-4.2.1/include/libavcodec/avcodec.h"
#include "../ffmpeg-4.2.1/include/libswscale/swscale.h"
#include "../ffmpeg-4.2.1/include/libswresample/swresample.h"
}
...
static char* av_ts2Str(int64_t ts)
{
	...
	return av_ts_make_string(ts_str, ts);
}

static char* av_tsto_timestr(int64_t ts, AVRational* tb)
{
	char ts_str[AV_TS_MAX_STRING_SIZE] = { 0 };
	return  av_ts_make_time_string(ts_str, ts, tb);
}
...
static void log_packet(const AVFormatContext* fmt_ctx, const AVPacket* pkt)
{
	...
	printf("pts:%s pts_time:%s dts:%s dts_time:%s duration:%s duration_time:%s stream_index:%d\n",
		av_ts2Str(pkt->pts), av_tsto_timestr(pkt->pts, time_base),
		av_ts2Str(pkt->dts), av_tsto_timestr(pkt->dts, time_base),
		av_ts2Str(pkt->duration), av_tsto_timestr(pkt->duration, time_base),
		pkt->stream_index);
}
	...
	enum AVCodecID codec_id)
{
	...
	/* 查找编码器 */
	*codec = const_cast<AVCodec*>(avcodec_find_encoder(codec_id));    //通过codec_id找到编码器
	if (!(*codec))
	{
	...
	*/
	...
	case AVMEDIA_TYPE_VIDEO: {
		 ...
		 * identical to 1. */
		AVRational time_base = { 1, STREAM_FRAME_RATE }; // 1/25
		ost->st->time_base = time_base;  // 时基
...
int muxing_flv_ffmpeg4(int argc, char** argv)
{
	...
	fmt = const_cast<AVOutputFormat*>(oc->oformat); // 获取绑定的AVOutputFormat
