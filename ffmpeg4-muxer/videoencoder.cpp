...
int VideoEncoder::InitH264(int width, int height, int fps, int bit_rate)
{
    ...
    codec_ctx_->flags |= AV_CODEC_FLAG_GLOBAL_HEADER; //这样就不会每帧都输出sps和pps了
...
//    av_dict_set(&dict_, "tune", "zerolatency", 0);
    ...
    frame_ = av_frame_alloc();//准备编码使用的AVFrame
    if(!frame_) {
...
AVPacket *VideoEncoder::Encode(uint8_t *yuv_data, int yuv_size, int stream_index, int64_t pts, int64_t time_base)
{
    ...
    //pts是原始yuv数据的时间维度信息是需要转换为编码器的time_base的，
    ...
    if(yuv_data) {
        ...
        ret = avcodec_send_frame(codec_ctx_, NULL);//flush encoder
    }
