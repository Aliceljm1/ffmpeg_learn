...
/**
* 通过fmt_ctx_创建流，注意新stream的codecpar需要从编码器上下文复制
*/
...
/***
* 1. packet时间基转换
* 2. av_interleaved_write_frame 写入文件
*/
int Muxer::SendPacket(AVPacket *packet)
{
    ...
    // 时间基转换， 因为packet是编码器输出的pts也是基于编码器的，写入文件时需要按照输出流的time_base，所以需要转换时间基
    ...
    //    ret = av_write_frame(fmt_ctx_, packet); // 立即写入文件，不排序，通常是直播流已经排序好的情况下使用
    av_packet_free(&packet);
    if(ret == 0) {
        return 0;
    } else {
