...
int audio_mix(int argc, char **argv)
{
    ...
    FILE *file1 = fopen("..\\res\\48000_2_f32le.pcm", "rb");
    if(!file1) {
    ...
    FILE *file2 = fopen("..\\res\\48000_2_s16le.pcm", "rb");
    if(!file2) {
        printf("fopen 48000_2_s16le.pcm failed\n");
        return -1;
    }

    FILE* file_out = fopen("..\\res\\audio_mix_output.pcm", "wb");
    if(!file_out) {
    ...
    while (1) {
        ...
        if (len1 > 0 || len2 > 0) {
            if (len1 > 0) {
                ...
                if(file1_finish == 0) {//有可能一路数据已经读完，但是另一路还没读完，所以要判断一下， 确保只调用一次
                    file1_finish = 1;
